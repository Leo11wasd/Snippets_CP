// LG el maximo log2(N) para N el tamaño máximo de cadena
const int LG = 22;
...
struct SuffixArray
{
    ...
    void costruct_lcp()
    {
        // El % es un separador para cuando se quieren contar substring distintos en un conjunto de varias cadenas.
        // solamente se concatenan con el simbolo % entre ellas.
        int k = 0;
        lcp.resize(n - 1, 0);
        for (int i = 0; i < n; i++)
        {
...
pair<int, int> isSubstring(string &s, vector<int> &suff_array, string &t)
{
    // devuelve primera y ultima ocurrencia en el suffix array del string t
    int n = s.size();
    int m = t.size();
    ...
    while (left <= right)
    {
        ...
        if (s.substr(suff_array[mid], m) == t)
        {
            ans = mid;       // t is prefix of this suffix
            right = mid - 1; // suffix > t
        }
        else if (s.substr(suff_array[mid], m) < t)
            left = mid + 1; // suffix < t
        else
            right = mid - 1; // suffix > t
    }
    ...
    {
        int mid = (left + right) / 2;
        // Compare t with the substring s[sa[mid] ... sa[mid]+m)
        if (s.substr(suff_array[mid], m) == t)
        {
            ans = mid;      // t is prefix of this suffix
            left = mid + 1; // suffix < t
