...
vector<PT> convex_hull(vector<PT> &p)
{
    ...
    for (auto &p : v)
    {
        // en lugar de >=0, poner >0 para incluir a puntos colineales
...
array<int, 2> convex_line_intersection(vector<PT> &p, PT a, PT b, int top)
{
    ...
    if (res[0] == res[1])
        return {res[0], -1}; // touches the vertex res[0]
...
double dist_from_point_to_polygon(vector<PT> &p, PT z)
{
    ...
    {
        for (int i = 0; i < n; i++)
            ans = min(ans, dist_from_point_to_seg(p[i], p[(i + 1) % n], z));
        return ans;
    }
...
// top - upper right vertex
double dist_from_polygon_to_line(vector<PT> &p, PT a, PT b, int top)
{ // O(log n)
    PT orth = (b - a).perp();
...
double get_maximum_polygon_area_for_given_lengths(vector<double> v)
{
    ...
    if (calc(r) <= 2 * PI - eps)
    { // the center of the circle is outside the polygon
        auto calc2 = [&](double r)
        {
            double sum = 0;
            for (int i = 0; i < v.size(); i++)
